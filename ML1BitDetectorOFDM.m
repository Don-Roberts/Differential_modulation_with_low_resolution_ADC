function decodedSym=ML1BitDetectorOFDM(Y,NSub, Nt,ModulationType, SNR)
            ...
            p;
               ...
               Yinst2signRealSignals = zeros(4,1);
                for u =1:U
                    ...
                    %Yinst1signRealSignals(1:2,1) = real(Yinst1);
                    %Yinst1signRealSignals(3:4,1) = imag(Yinst1);
                    
                    Yinst2signReal_1_2 = real(Yinst2sign);
                    Yinst2signimag_1_2 = imag(Yinst2sign);
                    ...
                    for ppp = 1:size(stored_symbols,2)
                        ...
                        symbolReal_1 = zeros(Nt*2,2) ;
                        symbolReal_1(1:2,1) = real(symbol(:,1)) .* Yinst2signReal_1_2(1);
                        symbolReal_1(3:4,1) = -1*imag(symbol(:,1)).* Yinst2signReal_1_2(1);

                        symbolReal_1(1:2,2) = imag(symbol(:,1)).* Yinst2signimag_1_2(1);
                        symbolReal_1(3:4,2) = 1*real(symbol(:,1)).* Yinst2signimag_1_2(1);
                        
                        newY_1 = (symbolReal_1.' * Yinst1signRealSignals);
                        
                        symbolReal_2 = zeros(Nt*2,2) ;
                        symbolReal_2(1:2,1) = real(symbol(:,2)).* Yinst2signReal_1_2(2);
                        symbolReal_2(3:4,1) = -1*imag(symbol(:,2)).* Yinst2signReal_1_2(2);

                        symbolReal_2(1:2,2) = imag(symbol(:,2)).* Yinst2signimag_1_2(2);
                        symbolReal_2(3:4,2) = 1*real(symbol(:,2)).* Yinst2signimag_1_2(2);
                        
                        newY_2 = (symbolReal_2.' * Yinst1signRealSignals);

                        
                        %fmin_mod(ppp) = fmin_mod(ppp) + sum(abs(Yinst2 - (symbol.' * Yinst1)).^2);

                        fmin_mod(ppp) = fmin_mod(ppp) + log(0.5*erfc( (-1/sqrt(2))*1*newY_1(1,:)))+...
                       log(0.5*erfc( (-1/sqrt(2))* 1*newY_1(2,:))) + log(0.5*erfc( (-1/sqrt(2))*1*newY_2(1,:)))+...
                       log(0.5*erfc( (-1/sqrt(2))* 1*newY_2(2,:))) ;
                    end
